...
#if defined(NDEBUG)
# error "Jppoints cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Jppoints Signed Message:\n";
...
{
    // Jppoints: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Jppoints: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Jppoints: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Jppoints: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jppoints: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jppoints: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Jppoints created on 2019";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04565ca5c10565a03824f413d45f23e57aada94d8538ecf41f13e68ee79985f3b9d18cd545280d41c88da1d6e54e7a2d011a6cb35e0fdae84823a887b9acbbac97") << OP_CHECKSIG;
        CBlock block;
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x36a3f625e3b346730a8fb6d41eebd4379ae48db624ab5889565869583ee1f2f2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc4, 0xbc, 0xd6 }; // Jppoints: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JppointsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JppointsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JppointsMiner(CWallet *pwallet)
{
    printf("JppointsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Jppoints-miner");
    ...
    try { loop {
        ...
        printf("Running JppointsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JppointsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JppointsMiner, pwallet));
}
